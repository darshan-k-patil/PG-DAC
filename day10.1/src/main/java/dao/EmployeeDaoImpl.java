...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public String addNewEmployee(Employee emp, long deptId) {
		...
		try {
			...
			Department dept = session.get(Department.class, deptId);//getting persistent dept
			if (dept != null) {
				// dept : PERSISTENT
...
//				emp.setAssignedDept(dept); // child --> parent'
				dept.addEmployee(emp);//establish bi directional relationship
				// save emp rec
			//	session.persist(emp); NO LONGER required : since cascade : ALL (includes update)
				mesg = "Added new emp to Dept " + dept.getDepartmentName();
			}
			tx.commit();//hibernate performs dirty chking --- finds state of dept  updated ---- finds new emp
			//in L1 cache --- auto fires insert query without explicitly add 
